...
public class SAOFranceUtils {
	  ...
	  private static CrashReporter crashReporter = new CrashReporter("SAOFrance Launcher", SAO_CRASH_DIR);
	    	...
	    	public void run() {
	    		...
	    		while(!isInterrupted()) {
	    			if(BarAPI.getNumberOfFileToDownload() == 0) {
	    				...
	    				Main.getInstance().getPanel().setStatus("Telechargement des fichers : " + val / 1000 + " / " + max / 1000 + "Mo");
	    				val = (int) (BarAPI.getNumberOfTotalDownloadedBytes() / 1000);
	    				max = (int) (BarAPI.getNumberOfTotalBytesToDownload() / 1000);
						Main.getPanel().getProgressBar().setValue(val);
	    				Main.getInstance().getPanel().getProgressBar().setMaximum(max);
	    			}
	    		}
	    	}
	    };
	...
	public static void update2() throws Exception  {
	  ...
	  public static void launch(String username) throws LaunchException {
		 	...
		 	if(java.equals("64")) profile.getVmArgs().addAll(Arrays.asList(Main.getInstance().getPanel().getProps().getRamArguments()));
		    ...
		    if(!javaVersion.contains("1.8")) {
		    	...
		    	JOptionPane.showMessageDialog(Main.getInstance().getPanel(), "Attention, vous n'etes pas sous Java 8. Vous risquez d'avoir des problemes au lancement.");
		    }
